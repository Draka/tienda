...
export class CartList {
  ...
  showDetailOrder() {
    const list = $('.cart-detail-order');
    if (list.length) {
      // existe el elemento

      const ids = Vars.getParameterByName('id') || '';
      list.find('.number-order').html(ids);
      this.getApi.g(`orders/id/${ids}`)
        .done((data: any) => {
          const $el = list.find('.order-detail');
          $el.html('');
          // Dibuja en la pantalla de vista de productos
          $.each(data.orders, (_i, elem) => {
            ...
            let desktop = '<table class="table table--striped hide-xs small">'
                  ...
                  + '</thead><tbody>';
              // Mobile
            let mobile = '<div class="show-xs">';

            $.each(elem.products, (_i, product) => {
              const img = Product.img(product, ['96x96', '196x196']);
              // Desktop
              desktop += '<tr>'
                    + `<td class="w-img-96">${img}</td>`
                    + `<td><div class="b">${product.name}</div></td>`
                    + `<td class="tr">${Vars.formatMoney(product.price)}</td>`
                    + `<td class="tr">${product.quantity}</td>`
              ...
              mobile += '<table class="table table--striped small">'
                      ...
                      + '<tr><th class="b">CANTIDAD</th>'
                      + `<td class="big">${product.quantity}</td></tr>`
                      + '<tr><th class="b">SUBTOTAL</th>'
                      + `<td class="b big t-secondary">${Vars.formatMoney(product.quantity * product.price)}</td></tr>`;
              mobile += '</table>';
            });

            desktop += '</tbody><tfoot><tr>'
                + '<td class="b" colspan="4">Subtotal</td>'
                + `<td class="tr t-secondary">${Vars.formatMoney(elem.order.subtotal)}</td>`
                + '</tr>'
                + '<tr>'
                + '<td class="b" colspan="4">Envío</td>'
                + `<td class="tr t-secondary">${Vars.formatMoney(elem.order.shipping)}</td>`
                + '</tr>'
                + '<tr>'
                + '<td class="b" colspan="4">Total</td>'
