const vars = {
  index: '',
  menu: '[menu]'
  + '[item]<a href="/paginas/precios">Precios</a>[/item][item]<a href="/ayuda">Ayuda</a>[/item][item]<a href="/usuario/vendedor">Vender</a>[/item]'
  + '[/menu]',
  'menu-mob': '[menu-mob]'
  + '[item]<a href="/paginas/precios">Precios</a>[/item][item]<a href="/ayuda">Ayuda</a>[/item][item]<a href="/usuario/vendedor">Vender</a>[/item]'
  + '[/menu]',
  'footer-l-1-c-1': '[logo-cuadrado]'
  + '<div class="small pt-1">TIENDAS EN PESOS COLOMBIANOS</div>',
  'footer-l-1-c-2': '',
  'footer-l-2-c-1': '<img class="h-2" src="https://cdn.vendelomejor.com/images/logo-p4s.svg" alt="p4s logo">'
  + '<div class="small">Apoyado por <a href="https://p4s.co">p4s</a></div>',
  'footer-l-2-c-2': '<div class="small">COMPRA 100% SEGURA</div>',
  'footer-l-2-c-3': '<img class="h-2" src="https://cdn.vendelomejor.com/images/logo-name.svg" alt="Véndelo Mejor logo blanco">'
  + '<div class="small">powered by: <a href="https://vendelomejor.com/">Véndelo Mejor</a></div>',
};

module.exports = vars;
